...
export const authApi = {
    login: (username: string, password: string) =>
        post<{ temp_token: string; needs_totp: boolean; needs_totp_setup: boolean }>(
            "/api/auth/login", { username, password }
        ),
    verifyTotp: (temp_token: string, code: string) =>
        post<{ token: string; username: string }>("/api/auth/verify-totp", { temp_token, code }),
    getTotpSetup: (temp_token: string) =>
        get<{ secret: string; uri: string; username: string }>(
            "/api/auth/totp-setup", { "X-Temp-Token": temp_token }
        ),
    confirmTotpSetup: (temp_token: string, code: string) =>
        post<{ token: string; username: string }>("/api/auth/totp-setup", { temp_token, code }),
    me: () => get<{ username: string; totp_enabled: boolean }>("/api/auth/me"),
    logout: () => post("/api/auth/logout", {}),
    changePassword: (current_password: string, new_password: string) =>
        post("/api/auth/change-password", { current_password, new_password }),
    disableTotp: () => post("/api/auth/disable-totp", {}),
}
